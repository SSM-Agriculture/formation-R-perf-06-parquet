...
Un exemple avec la fonction `arrow_table()` et son argument `schema` :

```{.r}
#Créez un DataFrame R 
df <- data.frame( 
  colonne1 = c(1, 2, 3), 
  colonne2 = as.character(c("A", "B", "C")), 
  colonne3 = as.logical(c(TRUE, FALSE, TRUE)),
  colonne4 = as.numeric(35.43, 29.93, 17.02)) |> 
  mutate(colonne4 = as.float64(colonne4))

# Définissez les types de colonnes souhaités 
schema <- schema(colonne1 = int32(), colonne2 = utf8(), 
                 colonne3 = boolean(), colonne4 = float64()) 


# Cette méthode crée un objet arrow::Table qui vous permet de spécifier explicitement le schéma des données
...
Les types supportés par arrow sont disponibles [sur cette page](https://arrow.apache.org/docs/r/reference/data-type.html).  

Lorsqu'on requête les types d'un fichier Parquet, voici [les réponses possibles](https://parquet.apache.org/docs/file-format/types/).
