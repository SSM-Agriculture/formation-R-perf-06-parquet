...
Pour vérifier que le chargement des données a bien fonctionné, la fonction `tbl()` permet d'accéder à une table de la base de données grâce à son nom.  
...
Comme précedemment la fonction `tbl()` peut être directement utilisée.  
...
La fonction `dbListTables()` est utile pour afficher les noms des tables présente dans une base de données.  
...
accéder aux métadonnée d'un fichier Parquet en utilisant la requête suivante :  
...
```{.r}
# On crée une nouvelle connexion + création d'une BDD vide avec le paramètre dbdir
con <- DBI::dbConnect(drv = duckdb::duckdb())
...
De même, les fonctions pivot_wider(), pivot_longer() et les windows_function existent nativement dans duckdb mais pas dans arrow.

Par exemple, arrow ne peut pas ajouter directement à la base permanente des équipements une colonne égale au nombre total d’équipements du département. Le code fonctionne en duckdb.
...
## Comment accéder aux colonnes dans l'onglet environnement (TODO)
