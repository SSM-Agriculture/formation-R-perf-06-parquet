...
Pour l'exemple, nous allons prendre une table des eploitations du RA
...
variables du fichier Parquet dont elle a besoin (en l'occurence
...
## Des infos utiles avec le package `{nanoparquet}` (1/2)
...
```{.r}
parquet_column_types("data/RA2020_exploitations.parquet")

# > A data frame: 255 × 6
   file_name                         name               type       r_type    repetition_type logical_type
 * <chr>                             <chr>              <chr>      <chr>     <chr>           <I<list>>   
 1 data/RA2020_exploitations.parquet NOM_DOSSIER        BYTE_ARRAY character OPTIONAL        <STRING>    
 2 data/RA2020_exploitations.parquet TYPE_QUESTIONNAIRE BYTE_ARRAY character OPTIONAL        <STRING>    
```

## Des infos utiles avec le package `{nanoparquet}` (2/2)
...
parquet_info("data/RA2020_exploitations.parquet")
parquet_metadata("data/RA2020_exploitations.parquet")
parquet_schema("data/RA2020_exploitations.parquet")
parquet_column_types("data/RA2020_exploitations.parquet")
```
...
-   Ecrire une fonction calculs_RA() qui - pour une région et une table
    ...
    (OLEAG_SUR_TOT) et enfin la part de la surface des cereales dans la
    SAU totale et la part de la surface des oléagineux dans la SAU
    totale.

-   Utilser ensuite la fonction calculs_RA() pour calculer ces
