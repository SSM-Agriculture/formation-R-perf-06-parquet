...
## Consulter les métadonnées d'un fichier Parquet (1/4)
...
## Consulter les métadonnées d'un fichier Parquet (2/4)

<br>

Le package [{nanoparquet}](https://nanoparquet.r-lib.org/index.html) permet aussi d'obtenir d'autres infos facilement depuis des fichiers parquet :
  
  
```{.r}
library(nanoparquet)

read_parquet_info("data/RA2020_exploitations.parquet")
...
Le package [{parquetize}](https://ddotta.github.io/parquetize/) contient aussi des [fonctions similaires](https://ddotta.github.io/parquetize/reference/get_parquet_info.html) :

```{.r}
library(parquetize)

get_parquet_info("data/RA2020_exploitations.parquet")

# > # A tibble: 1 × 5
  path                              num_rows num_row_groups num_columns mean_row_group_size
  <chr>                                <int>          <int>       <int>               <dbl>
1 data/RA2020_exploitations.parquet   416478              1         255              416478
```

## Consulter les métadonnées d'un fichier Parquet (3/4)

- Les types des colonnes :  

```{.r}
nanoparquet::read_parquet_schema("data/output.parquet")
...
## Consulter les métadonnées d'un fichier Parquet (4/4)

La fonction `read_parquet_schema()` retourne les types de **bas niveau** dans la colonne `type` et les **logical_type** dans la colonne `logical_type`.

- Les types `bas niveau` : c'est la manière dont les données sont stockées physiquement dans le fichier.  
- Les `logical types` : c'est une interprétation sémantique des données stockées (ils donnent du sens aux types bas niveau).   
  
  
**Pour aller plus loin sur les métadonnées d'un fichier :**  
...
-   Dans votre espace de travail, créer les 3 fichiers parquet suivants
    :
...
Ajouter la colonne `classe` issue de la table `data_c` dans le tableau
final.
